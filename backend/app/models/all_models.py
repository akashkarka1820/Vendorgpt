...
class Customer(Base):
    ...
    khata_entries = relationship("KhataTransaction", back_populates="customer")
...
class Transaction(Base):
    ...
    payment_method = Column(String(50), nullable=False, default="Cash")  # Cash, UPI, Card, Khata
...
class KhataTransaction(Base):
    ...
    reference_invoice_id = Column(Integer, ForeignKey("transactions.id"), nullable=True)
...
class InventoryMovement(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    product_id = Column(Integer, ForeignKey("products.id"), nullable=False)
    change_type = Column(String(20), nullable=False)  # DEDUCTION, ADDITION, ADJUSTMENT
    quantity = Column(Float, nullable=False)
    reference_id = Column(String(50), nullable=True)  # invoice number or manual ref
